"""MyRetroTVs decade streams via Chrome DevTools Protocol screencast.

Launches headless Chrome at ``https://{era}s.myretrotvs.com/``, hides the
decorative TV chrome so only the video fills the viewport, and captures frames
...
class RetroTvChannel:
    ...
    def _start_once(self, chrome_path: str) -> bool:
        ...
        LOG.warning(
            "Retro TV: no frames after %.1fs (%s)",
    ...
    _JS_WATCHDOG = r"""
(() => {
  const out = {power: false, crop: true, on: false};
  const hideSels = [
